...
def implement_execute_tool_call(tool_call: Dict, available_tools: List[Tool]) -> str:
    ...
    tool_args = tool_call.get("args", {"text": ""})
    ...
    observation = tool_to_use.invoke(tool_args.get("text"))
...
def implement_run_agent_with_tool_calling(model_with_tools: ChatOpenAI, 
    ...
    # YOUR CODE HERE
    while True:
      ai_message = model_with_tools.invoke(user_input)
      if implement_check_for_tool_calls(ai_message) == True:
        tool_calls = getattr(ai_message, "tool_calls")
        result = implement_execute_tool_call(tool_calls[0], available_tools)
        return result
      else:
